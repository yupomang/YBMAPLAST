...
public class BSPDesTest {

	private static int pc_1_cp[] = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42,
			34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36 };
	private static int pc_1_dp[] = { 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46,
			38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4 };
	private static int pc_2p[] = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
			23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55,
			30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29,
			32 };

	private static int ls_countp[] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2,
			2, 2, 1 };
	...
	private static int iip_tab_p[] = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52,
			...
			23, 15, 7 };
	private static int _iip_tab_p[] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7,
	...
	private static int e_r_p[] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9,
			10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20,
			21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

	private static int local_PP[] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23,
	 ...
	 ***/
	private static byte[] reportdes(byte[] souchar, byte[] key, int len,
	...
	private static void asc_to_bcd(byte[] bcd_buf, byte[] ascii_buf,
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static byte[] encryption(String msg, String key)
			throws UnsupportedEncodingException {
		// [padding模式]:明文长度,不是8的倍数的,需要用空格补齐
		if (msg.getBytes().length % 8 != 0) {
			msg = msg + "        ".substring(msg.getBytes().length % 8);
		}
		byte[] ret = reportdes(msg.getBytes(), key.getBytes(),
				msg.getBytes().length, 0);
		return ret;
	}
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static String decryption(byte[] msg, String key)
			throws UnsupportedEncodingException {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		// 密钥的加密解密
		byte[] encKey = BSPDesTest.keydes("12345678".getBytes(),
				"12345678".getBytes(), 8, 0);
		String encKeyS = new String(encKey);
		System.out.println("秘钥加密后为" + encKeyS);
		byte[] decKey = BSPDesTest.keydes(encKeyS.getBytes(),
				"12345678".getBytes(), 8, 1);
		System.out.println("秘钥为" + new String(decKey));
		...
		// 密码加密
		String s = "003030";
		byte[] m = BSPDesTest.encryption(s, key);
		byte[] n = bcd_to_asc(m);
		System.out.println(s + "加密后的数据库中的密码为" + new String(n));

		// 密码解密
		String acpwd = "6591EF7278D3C27F";
		byte[] j = acpwd.getBytes();
		byte[] k = asc_to_bcd(j, j.length);
		System.out.println(acpwd + "解密后的密码为" + BSPDesTest.decryption(k, key));
	}
}
