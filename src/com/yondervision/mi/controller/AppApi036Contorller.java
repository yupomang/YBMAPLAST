...
public class AppApi036Contorller {
	...
	public String appapi03601(AppApi030Form form,  ModelMap modelMap, HttpServletRequest request, 
...
//		pc.check(form);
	...
	public String appapi03602(AppApi030Form form,  ModelMap modelMap, HttpServletRequest request, 
			HttpServletResponse response) throws Exception{
		Logger log = LoggerUtil.getLogger();
		form.setBusinName("电子还款处理");	
		log.info(LOG.START_BUSIN.getLogText(form.getBusinName()));
		log.debug(DEBUG.SHOW_PARAM.getLogText(CommonUtil
				.getStringParams(form)));
		ApiUserContext.getInstance();
...
//			modelMap.clear();
//			modelMap.put("recode", "999999");
//			modelMap.put("msg", "客户 未高级认证！");
//			return "";
//		}
		Mi048 mi0481 = webApi029ServiceImpl.webapi02919(mi029 , "01" ,request ,response);
		m.put("accnum", mi0481.getAccountcode());
		m.put("bodyCardNumber", mi029.getCertinum());
//		Mi048 mi0482 = webApi029ServiceImpl.webapi02919(mi029 , "04" ,request ,response);
//		m.put("lncontrnum", mi0482.getAccountcode());
		m.put("bodyCardNumber", mi029.getCertinum());
		...
		System.out.println("商贷还款返回信息："+rep);
		...
		if("000000".equals((String)hasMap.get("recode"))){
			System.out.println("appapi03602资金类业务处理结果，是否记账：【"+((String)hasMap.get("acc_flag"))+"】");
			System.out.println("appapi03602资金类业务处理结果，记账金额：【"+((String)hasMap.get("acc_amt"))+"】");
	...
	public String appapi03603(AppApi030Form form,  ModelMap modelMap, HttpServletRequest request, 
			HttpServletResponse response) throws Exception{
		Logger log = LoggerUtil.getLogger();
		form.setBusinName("冲还贷处理");	
		log.info(LOG.START_BUSIN.getLogText(form.getBusinName()));
		log.debug(DEBUG.SHOW_PARAM.getLogText(CommonUtil
				.getStringParams(form)));
		ApiUserContext.getInstance();
...
//		m.put("lncontrnum", mi0482.getAccountcode());
		...
		System.out.println("公积金还款返回信息："+rep);
		
		HashMap hasMap = (HashMap) JsonUtil.getDisableHtmlEscaping().fromJson(rep, HashMap.class);
		if("000000".equals((String)hasMap.get("recode"))){
			System.out.println("appapi03603资金类业务处理结果，是否记账：【"+((String)hasMap.get("acc_flag"))+"】");
			System.out.println("appapi03603资金类业务处理结果，记账金额：【"+((String)hasMap.get("acc_amt"))+"】");
			request.setAttribute("acc_flag", (String)hasMap.get("acc_flag"));
			request.setAttribute("acc_amt", (String)hasMap.get("acc_amt"));
		}
