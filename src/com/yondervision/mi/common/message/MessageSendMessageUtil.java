...
* 
*/ 
public class MessageSendMessageUtil {
	
	public void toAPPServer(String urlString,String value,HttpServletRequest request, HttpServletResponse response) throws TransRuntimeErrorException{
		StringBuffer buffer = new StringBuffer();
		try {			
			URL url = new URL(urlString.trim());
			HttpURLConnection httpUrlConn = (HttpURLConnection) url.openConnection();			
			System.out.println("调用前置通信服务接口:"+urlString);
...
//			}
			...
			System.out.println("********  短消息通信返回信息："+msg);
			if(request!=null){
...
//			modelMap.put("errormsg", "服务字符编码异常");
		} catch (IOException e) {
			e.printStackTrace();
			throw e;
//			modelMap.put("errorcode", "103");
//			modelMap.put("errormsg", "IO异常");
		}catch (Exception e){
			e.printStackTrace();
			throw e;
//			modelMap.put("errcode", "104");
//			modelMap.put("errmsg", "通讯异常");
		}finally{
    ...
    public String post(String url, String data,HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			post.setEntity(entity);
			HttpResponse response1;
			response1 = new MessageSendMessageUtil().getClient().execute(post);
			if (response1.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				System.out.println();
			}
			String encoding = null;
