...
public class Weixindenglu {
	...
	public static void httpURLConnectionPOST () {  
		try {  
			...
			//userId必须加密处理
			String userId = aes.encrypt("330204197712205026".getBytes("UTF-8"));
			...
			String certinum = aes.encrypt("330204197712205026".getBytes("UTF-8"));			
			String accnum = aes.encrypt("".getBytes("UTF-8"));	
			String pwd = aes.encrypt("111111".getBytes("UTF-8"));	

			//用于数字签名			
			String parm ="centerId=00057400&userId="+userId+"&usertype=10&deviceType=3&deviceToken=&currenVersion=1.0&buzType=5432&devtoken=&channel=20&appid="+appId+"&appkey="+appKey+"&appToken=&clientIp=DJv9mNfY9qPBA/x6cL6iFQ==&bodyCardNumber="+certinum+"&password="+pwd+"&accnum="+accnum;
			//用于发送http报文
			String parm1 ="centerId=00057400&userId="+userId.replace("+","%2B")+"&usertype=10&deviceType=3&deviceToken=&currenVersion=1.0&buzType=5432&devtoken=&channel=20&appid="+appId.replace("+", "%2B")+"&appkey="+appKey.replace("+", "%2B")+"&appToken=&clientIp=DJv9mNfY9qPBA/x6cL6iFQ==&bodyCardNumber="+certinum.replace("+","%2B")+"&password="+pwd.replace("+","%2B")+"&accnum="+accnum.replace("+","%2B");
