...
(function ($) {
	...
	function show(target) {
		...
		$(target).find('.calendar-title span').html(opts.months[opts.month - 1] + ' ' + opts.year + '【' + Animals[(opts.year - 4) % 12] + '年】');
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				var day = week[j];
				var dayHtml = '<font size="4" face="Arial Black">' + day[2];
				var info = null;
				if (opts.month == day[1]) {
					info = currentCa[day[2] - 1];
					if (info.color) {
						var color = 'color="' + info.color + '"';
						dayHtml = '<font size="4" face="Arial Black" ' + color + '>' + day[2];
					}
					dayHtml += '</font>';
					if (opts.lunarDay) {
						dayHtml += "<br/>";
						if(day[4] == null) day[4] = '';
						if(day[4].length>0){
							if(day[4].indexOf("/")>0&&day[4].split("/")[0]!=day[4].split("/")[1]){
								dayHtml += '<font size="3" color="black" style="font-size:9pt">' + day[4] + '</font>';
							}else{
								dayHtml += '<font size="3" color="blue" style="font-size:9pt">' + day[4] + '</font>';
							}
						}else{
							if (opts.solarTerms && info.solarTerms) {
								dayHtml += '<font color="limegreen">' + info.solarTerms + '</font>';
							} else {
								dayHtml += '<font size="3" style="font-size:9pt">' + cDay(info.lDay, info.lMonth) + '</font>';
							}
						}
				...
				var dayO = $('<td class="calendar-day fullcalendar-day calendar-other-month"></td>').data('info', info).attr('abbr', day[0] + ',' + day[1] + ',' + day[2] + ',' + day[3] + ',' + day[4]).html(dayHtml).appendTo(tr);

				if(day[3]==1){
					dayO.attr("bgcolor","#f4f5f9");
				}else{
					dayO.removeAttr("bgcolor");
				}
				if (info && (info.lunarFestival || info.solarFestival)) {
					dayO.addClass('festival');
				}
				/*dayO.hover(function (e) {
				 clearTimeout(hideTimer);
		...
		if (opts.current) {
			...
			t.find('td[abbr="' + current + '"]').addClass('calendar-selected').css('backgroundColor', '#33cc66');
		}
		...
		t.find('td').hover(
			...
			t.find('.calendar-selected').removeClass('calendar-selected').css('backgroundColor', '');
			$(this).addClass('calendar-selected').css('backgroundColor', '#33cc66');
	...
	/*
	 * 构造插件内容（一个月及前后） get weeks data.
	 */
	function getWeeks(target, year, month) {
		//alert("getWeeks-["+year+"]-["+month+"]");
		var opts = $.data(target, 'fullCalendar').options;
		var dates = [];
		var rows=[];
		...
		opts.para.yearmonth=year+"-"+monthStr;
		$.ajax( {
			type : "POST",
			url : opts.url,
			dataType: "json",
		...
		for (var i = 1; i <= lastDay; i++){
			dates.push([year, month, i,rows[i-1].festivalflag,rows[i-1].freeuse2]);
		}
		// group date by week
		var weeks = [],
			week = [];
		...
		if (firstWeek.length!=0&&firstWeek.length < 7) {
	...
	function showMonth(target, delta) {
		var opts = $.data(target, 'fullCalendar').options;
		opts.month=parseFloat(opts.month);
		opts.month += delta;
		if (opts.month > 12) {
			opts.year++;
			opts.month = 1;
	...
	function showYear(target, delta) {
		var opts = $.data(target, 'fullCalendar').options;
		opts.year=parseFloat(opts.year);
		opts.year += delta;
		show(target);
