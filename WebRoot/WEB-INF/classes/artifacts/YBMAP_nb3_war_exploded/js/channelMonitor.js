//渠道管理 - 渠道监控配置 
//xingmc 
//2016年8月8日
/**
 * Modify by M on 2016/9/2.
 */
var user = top.userInfo;
//初始化分页信息
var appm = {
	pager:null,
	tableObj:null,
	createPager: function(){
		var _this = this;
		_this.pager = pages({
            el: "#channael-table-pages",
            itemLength: 2300,
            pageSize: 12,
            pageChanged: function (pageIndex) {
                init(user.centerid,false,pageIndex);
            }
        });

        init(user.centerid,false,1);
	},
	customerName:'',  //客户名称
	channelName:'',   //渠道名称
};


//所有的错误信息
function errMsg (){
	parent.Common.dialog({
		type:'error',
		text:'网络错误！请刷新页面或联系管理员！',
		isShow:true,
		cancelShow:true,
		okText:'确定',
		ok:function(){
			
		}
	});
}

var appManage = new Vue({
	el:'#cam',
	data:{
		isAdd:true,
		//title:['应用管理 - 增加','应用管理 - 修改'],
		form:{
			customerName:'',	//客户名称
			channelName:'',		//渠道名称
			appName:'',			//应用名称
			isUsing:'',			//是否启用
			serviceStart:'',	//服务开始时间
			serviceEnd:'',		//服务结束时间
		},
		authForm:{
			appid:'',
			appkey:'',
			appaes:'',
			apptoken:'',
			apptokentext:'',
			tokendate:''
		}
	},
	methods:{
		clearForm: function(){
			var _this = this;
			_this.form.customerName = '';
			_this.form.channelName  = '';
			_this.form.appName		= '';
			_this.form.isUsing		= '';
			_this.serviceStart		= '';
			_this.serviceEnd		= '';
		},
		clearAuth: function(){
			var _this = this;
			_this.authForm.appid = '';
			_this.authForm.appkey  = '';
			_this.authForm.appaes		= '';
			_this.authForm.apptoken		= '';
			_this.authForm.apptokentext		= '';
			_this.authForm.tokendate		= '';
		}
	}
});

//获取全局的中转移
var centeridObj = [];
var channelObj = [];
var appnameObj = [];

var centeridData = null;
var channelData = null;
var appnameData = null;
var pageData = null;

var appNameHtml = '';
var channelHtml = '';

//第一次加载的时候，从数据库读取客户名称
function getAll(){
	parent.Common.loading(true);
	$.ajax({
		type:'POST',
		url:'./webappcomCenterId.json',
		data:{},
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			centeridData = data;
			if(data.recode == '000000'){
				var customerOptions = '';
				for(var i = 0;i<data.mi001list.length;i++){
					if(user.centerid != '00000000'){
						if(user.centerid == data.mi001list[i].centerid){
							customerOptions = '<option value="'+data.mi001list[i].centerid+'">'+data.mi001list[i].centername+'</option>'
						}
					} else {
						customerOptions += '<option value="'+data.mi001list[i].centerid+'">'+data.mi001list[i].centername+'</option>';
					}
					centeridObj.push(data.mi001list[i]);
				}
				$('#customerName').html(customerOptions);
				$('#searchCustomer').html(customerOptions);

				// 切换的时候 发请求 去get渠道
				$('#searchCustomer').off().on("change", function () {
					getChannel($(this).val(),false);
				});

				
				appm.createPager();
				getAllChannelType();

				getChannel($('#searchCustomer').val(),false);
				
			}
			
			parent.Common.loading(false);
		},
		error:function(){
			errMsg();
		}
	});
	
	
	
}


//画表格头部mi041
var cols = [
	{title:'序号', name:'centerid' ,width:80, align:'center',renderer: function(val,item,index){
		return index+1;
	}},
   { title:'客户名称', name:'centerid' ,width:150, align:'center',renderer: function(val,item,index){
   		for(var i = 0;i<centeridObj.length;i++){
	   		if(item.centerid == centeridObj[i].centerid){
	   			return centeridObj[i].centername;
	   		}
	   	}
   } },
   { title:'渠道类型', name:'channel' ,width:180, align:'center', renderer: function(val,item,index){
   		for(var i = 0;i<channelObj.length;i++){
	   		if(item.channel == channelObj[i].itemid){
	   			return channelObj[i].itemval;
	   		}
	   	}
   } },
   { title:'渠道应用', name:'appname' ,width:180, align:'center' },
   { title:'是否监控', name:'monitor' ,width:80, align:'center',renderer: function(val ,item, index){
   		if(item.monitor == '1'){
   			return '是';
   		}
   		if(item.monitor == '0'){
   			return '否';
   		}
   }},
   { title:'心跳地址', name:'checkurl' ,width:325, align:'center'},
];

//init page
function init(centerid,changer,pageIndex){
	$.ajax({
		type:'POST',
		url:'./webapi04104.json',
		data:{'centerid':centerid,'page':pageIndex,'rows':'10'},
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			pageData = data;
			if( appm.tableObj != null){
				appm.tableObj.load(data.rows);
			} else {
				appm.tableObj = $('#changeMonitorTable').mmGrid({
					multiSelect: true,// 多选
					checkCol: true, // 选框列
					height: 'auto',
					cols: cols,
					items: data.rows,
					loadingText: "loading...",
					noDataText: "暂无数据。",
					loadErrorText: "数据加载异常，请刷新页面重试，或联系网站管理员。",
					sortable: false
				});	
			}
			
			//修改分页
			appm.pager.reset({
				itemLength:data.total,
	            pageSize:data.pageSize,
	            reset:changer?true:false
			});
			//绑定按钮事件
			btnBindEvent();
		},
		error:function(){
			errMsg();
		}
	});
}




//清除表单内容
function clearForm(){
	$('#customerName option').attr('selected',false);
	$('#channelName option').attr('selected',false);
	$('#appName option').attr('selected',false);
	$('.radioWrap input').removeAttr('checked');
	$('#checkUrl').attr('value','');
}

//点击添加
function create(){
	appManage.isAdd = true;
	appManage.clearForm();
	setTimeout(function(){
		var createHTML = $('.orgManage-popup-edit-container').html();
		parent.Common.popupShow(createHTML);

		var centerid = parent.$('#customerName').val();
		parent.$('#customerName').off().on("change", function () {
			centerid = $(this).val();
		});
		parent.$('#channelName').off().on("change", function () {
			getApp(centerid, $(this).val(), true);
		});
	},50);
	$('#addTitltSpan').text('渠道监控配置 - 增加');
	clearForm();
	$('.radioWrap input:first').attr('checked','checked');
	$('#appName').html('');
	$('.orgManage-popup-btns-ok:eq(0)').show();
	$('.orgManage-popup-btns-ok:eq(1)').hide();	
}
//点击修改
function modify(){
	appManage.isAdd = false;
	var selectedItem = appm.tableObj.selectedRows(),
		centerid = parent.$('#customerName').val(),
		_centerid = selectedItem[0].centerid,
		_channel = selectedItem[0].channel;

	parent.Common.loading(true);
	setTimeout(function(){
		var editHTML = $('.orgManage-popup-edit-container').html();
		parent.Common.popupShow(editHTML);

		parent.$('#customerName').off().on("change", function () {
			centerid = $(this).val();
		});
		parent.$('#channelName').off().on("change", function () {
			var selectedItem = appm.tableObj.selectedRows(),
				centerid = parent.$('#customerName').val();
			getApp(centerid, $(this).val(), true, selectedItem[0].pid);
		});

	},50);
	clearForm();
	// getAppForModify(selectedItem[0].centerid,selectedItem[0].channel);

	//回显
	$('#customerName option[value="'+_centerid+'"]').attr('selected',true);
	$('#channelName option[value="'+_channel+'"]').attr('selected',true);
	$('#appName option[value="'+selectedItem[0].pid+'"]').attr('selected',true);
	if(selectedItem[0].monitor == '1'){
		$('.radioWrap input[name="isUsing"]:first').attr('checked','checked');
		$('.radioWrap input[name="isUsing"]:last').attr('checked',false);
	} else {
		$('.radioWrap input[name="isUsing"]:last').attr('checked','checked');
		$('.radioWrap input[name="isUsing"]:first').attr('checked',false);
	}
	$('#checkUrl').attr('value',selectedItem[0].checkurl);
	$('#addTitltSpan').text('渠道监控配置 - 修改');
	//按钮显示隐藏
	$('.orgManage-popup-btns-ok:eq(0)').hide();
	$('.orgManage-popup-btns-ok:eq(1)').show();
	
}



var setForm = {
	verify:function(isAdd){
		var _this = this;
		var post = true;
		var dialogErrMsg = [];
		if(parent.$('#customerName').val() == ''){
			post = false;
			dialogErrMsg.push('客户名称');
		}
		if(parent.$('#channelName').val() == ''){
			post = false;
			dialogErrMsg.push('渠道名称');
		}
		if(parent.$('#appName').val() == ''){
			post = false;
			dialogErrMsg.push('应用名称');
		}
		if(parent.$('.radioWrap').find('input[name="isUsing"]:checked').val() == ''){
			post = false;
			dialogErrMsg.push('是否认证');
		}
		if(parent.$('#checkUrl').val() == '' && parent.$('.radioWrap').find('input[name="isUsing"]:checked').val() == "1"){
			post = false;
			dialogErrMsg.push('心跳地址');
		}else if(!an.isURL(parent.$('#checkUrl').val()) && parent.$('.radioWrap').find('input[name="isUsing"]:checked').val() == "1"){
			parent.$('#popup-container').hide();
			parent.Common.dialog({
				type:'warning',
				text: '心跳地址格式不正确，请检查后重新填写！',
				okShow:true,
				cancelShow:false,
				okText:'确定',
				ok:function(){
					parent.$('#popup-container').show();
				}
			});
			return;
		}
		if(!post){
			var errmsg = dialogErrMsg.join(',');
			if(dialogErrMsg.length > 1){
				errmsg+='等';
			}
			parent.$('#popup-container').hide();
			
			parent.dialogBox.type('error');
			parent.dialogBox.text(errmsg + '不能为空');
			parent.dialogBox.ok(function(){
				parent.$('#popup-container').show();
			});
			parent.dialogBox.show();
		} else {
			parent.Common.loading(true);
			var postData = {
				centerid:parent.$('#customerName').val(),
				channel:parent.$('#channelName').val(),
				pid:parent.$('#appName').val(),
				monitor:parent.$('.radioWrap').find('input[name="isUsing"]:checked').val(),
				checkurl:parent.$('#checkUrl').val(),
			}
			if(isAdd){
				createData(postData);
			} else {
				itemModify();
			}
		}
	},
	search: function(){
		var searchData = {
			'centerid':$('#searchCustomer').val(),
			'channel':$('#searchChannel').val(),
			'pid':$('#searchAppname').val(),
			'page':1,
			'rows':'10'
		};
		$.ajax({
			type:'post',
			url:'./webapi04104.json',
			data: searchData,
			datatype:'json',
			success: function(data){
				appm.tableObj.load(data.rows);

				//修改分页
				appm.pager.reset({
					itemLength:data.total,
		            pageSize:data.pageSize,
		            reset:true
				});
			},
		});
	},
}

//添加
function createData(createData){
	var centerid = $('#searchCustomer').val();
	$.ajax({
		type:'POST',
		url:'./webapi04101.json',
		data:createData,
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			parent.Common.loading(false);
			if(data.recode == '000000'){
				parent.$('#popup-container').hide();
				
				parent.Common.dialog({
					type:'success',
					text:data.msg,
					okShow:true,
					cancelShow:false,
					okText:'确定',
					ok:function(){
						//添加成功之后重新执行查询操作
						init(centerid,false,1);
					}
				});
			} else {
				parent.$('#popup-container').hide();
				parent.Common.dialog({
					type:'error',
					text:data.msg,
					okShow:true,
					cancelShow:false,
					okText:'确定',
					ok:function(){}
				});
			}
		},
		error:function(){
			errMsg();
		}
	});
}



//确认是否删除事件
function deleteConfirm (){
	var selected = appm.tableObj.selectedRows();
	if(selected != 'undefined'){
		parent.Common.dialog({
            type: "warning",
            text: "确定要删除选中项吗？",
            okShow: true,
            cancelShow: true,
            okText: "确定",
            cancelText:'取消',
            ok: function () {
            	itemDelete();
            }
        });
	}
}

//删除事件
function itemDelete(){
	var selected = appm.tableObj.selectedRows(),
		centerid = $('#searchCustomer').val(),
		selarr = [];
	//获取选中的数组
	for(var i = 0 ;i < selected.length ;i++){
		selarr.push(selected[i].monitorid);
	}
	//数组转换成字符串
	selarr = selarr.join(',');
	if(selected.length < 1){
		parent.Common.dialog({
            type: "error",
            text: "至少选中一条记录！",
            okShow: true,
            cancelShow: false,
            okText: "确定",
            ok: function () {}
        });
	} else {
		//显示Loading
		parent.Common.loading(true);
		$.ajax({
			type:'POST',
			url:'./webapi04102.json',
			data:{ 'monitorid':selarr },
			datatype:'json',
			success:function(data){
				if(typeof data == 'string'){
					data = JSON.parse(data);
				}
				if(data.recode = '000000'){
					//隐藏loading
					parent.Common.loading(false);
					init(centerid,false,1);
				} else {
					parent.Common.loading(false);
					parent.Common.dialog({
						type:'error',
						text:data.msg,
						okShow:true,
						cancelShow:false,
						okText:'确定',
						ok:function(){
							
						}
					});
				}
			},
			error:function(){
				errMsg();
			}
		});
	}
}



//修改事件
function itemModify(){
	parent.Common.loading(true);
	//执行修改操作
	var selectedItem = appm.tableObj.selectedRows(),
		centerid = $('#searchCustomer').val(),
		itemModifyData = {
			monitorid:selectedItem[0].monitorid,
			centerid:parent.$('#customerName').val(),
			channel:parent.$('#channelName').val(),
			pid:parent.$('#appName').val(),
			monitor:parent.$('.radioWrap').find('input[name="isUsing"]:checked').val(),
			checkurl:parent.$('#checkUrl').val(),
		};
	$.ajax({
		type:'post',
		url:'./webapi04103.json',
		data:itemModifyData,
		datatype:'json',
		success: function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			if(data.recode = '000000'){
				//隐藏loading
				parent.Common.loading(false);
				parent.$('#popup-container').hide();
				parent.Common.dialog({
					type:'success',
					text:data.msg,
					okShow:true,
					cancelShow:false,
					okText:'确定',
					ok:function(){
						//添加成功之后重新执行查询操作
						init(centerid,false,1);
					}
				});
			} else {
				parent.Common.loading(false);
				parent.Common.dialog({
					type:'error',
					text:data.msg,
					okShow:true,
					cancelShow:false,
					okText:'确定',
					ok:function(){
						
					}
				});
			}
		},
		error: function(){
			errMsg();
		}
	});
	
}


//按钮绑定
function btnBindEvent (){
	$('#orgManage-btn-add').off().on('click',function(){
		create();
	});
	
	$('#orgManage-btn-modify').off().on('click',function(){
		var selectedItem = appm.tableObj.selectedRows();
		if(selectedItem.length < 1) {
            parent.Common.dialog({
                type: "error",
                text: "请至少选择一条记录进行修改！",
                okShow: true,
                cancelShow: false,
                okText: "确定",
                ok: function () {}
            });
            return;
        } 
        if(selectedItem.length>1){
            parent.Common.dialog({
                type: "error",
                text: "只能选择一条记录进行修改！",
                okShow: true,
                cancelShow: false,
                okText: "确定",
                ok: function () {}
            });
            return;
        }
		var _centerid = selectedItem[0].centerid,
			_channel = selectedItem[0].channel;
		getApp(_centerid,_channel,true, selectedItem[0].pid);
		//创建修改html
		modify();

	});
	$('#orgManage-btn-del').off().on('click',function(){
		var selectedItem = appm.tableObj.selectedRows();
		if(selectedItem.length == 0){
			parent.Common.dialog({
				type: "error",
	            text: "请选择想要删除的记录！",
	            okShow: true,
	            cancelShow: false,
	            okText: "确定",
	            ok: function () {
	            }
			});
			return ;
		} else {
			deleteConfirm();
		}
		
	});
	
}



function getAppName(pid){
	for(var i=0;i<appnameData.applist.length;i++){
		if(pid == appnameData.applist[i].pid){
			return appnameData.applist[i].appname;
		}
	}
}



$(function(){
	//初始化页面
	getAll();
	
	
});


// 获取应用列表
function getApp(centerid, itemid, isPopup, pid) {
	$.ajax({
		type:'POST',
		url:'./webapi04007.json',
		data:{
			'centerid': centerid,
			'channel': itemid
		},
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			appnameData = data;
			if(data.recode == '000000'){
				var appnameOptions = '<option value="">请选择</option>';
				for(var i =0;i<data.applist.length;i++){
					appnameOptions += '<option value="'+data.applist[i].pid+'">'+data.applist[i].appname+'</option>';
					appnameObj.push(data.applist[i]);
				}
				if(isPopup) {
					console.log("run this", appnameOptions);
					setTimeout(function () {
						parent.$('#appName').html(appnameOptions);
						if(pid) {
							parent.$('#appName option[value="'+ pid +'"]').attr('selected',true);
						}
					}, 100);
					console.log(parent.$('#appName').length);
				} else {
					$('#appName').html(appnameOptions);
					$('#searchAppname').html(appnameOptions);
					appNameHtml = appnameOptions
				}
				parent.Common.loading(false);
			} else {
				parent.Common.loading(false);
			}
		},
		error:function(){
			errMsg();
		}
	});
}


function getChannel (centerid,isPopup) {

	$.ajax({
		type:'POST',
		url:'./webappcomChannel.json',
		data: { 'centerid': centerid },
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			channelData = data;
			if(data.recode == '000000'){

				var channelOptions = '<option value="">请选择</option>';
				for(var i =0;i<data.mi007list.length;i++){
					channelOptions += '<option value="'+data.mi007list[i].itemid+'">'+data.mi007list[i].itemval+'</option>';
					channelObj.push(data.mi007list[i]);
				}
				if(isPopup) {
					parent.$('#channelName').html(channelOptions);
					
				} else {
					$('#channelName').html(channelOptions);
					$('#searchChannel').html(channelOptions);
					channelHtml = channelOptions;
				}
			}
			// 获取应用列表
			// getApp(centerid, channelObj[0].itemid, false);
			// 切换的时候 发请求 去get应用列表
			$('#searchChannel').off().on("change", function () {
				getApp(centerid, $(this).val(), false);
			});

		},
		error:function(){
			errMsg();
		}
	});
}
function getAllChannelType() {
	// 取所有渠道类型
	parent.Common.loading(true);
	$.ajax({
		type:'POST',
		url:'./webappcomChannel.json',
		data:{},
		datatype:'json',
		success:function(data){
			if(typeof data == 'string'){
				data = JSON.parse(data);
			}
			if(data.recode == '000000'){
				channelObj = data.mi007list;
				//init(false,1,10);
			}

		},
		error:function(){
			errMsg();
		}
	});
}