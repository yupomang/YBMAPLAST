...
$(function() {
	...
	$('#infoShow').click(function (){ 
		$('#dlg-phoneShow').html('<div id="pinfoShow" ><div class="phonebg"><div class="screenbg"><h1 class="item-title">短消息</h1><div id="content" class="item-content"><div id="phoneNo">客服电话：<%=request.getAttribute("custsvctel") %></div><div id="phoneNo">消息标题：<label id="infoTitle"></label></div><div id="phoneNo">消息描述：<h3 id="infoDetailms"></h3></div><div id="phoneNo">以下为消息正文：</div><div><h3 id="infoDetail"></h3></div></div></div></div></div>');
		var title = $('#title').val(); 
		...
		*/
		...
		$('#infoDetail').html(detail == ''||detail == null ? '< 预览消息内容处 >': detail);
		var infoDetailms = $('#detail').val().replace(/\n/g, '<br>').replace(/\t/g, '　　　　').replace(/ /g, '&nbsp;');
		
		$('#infoDetailms').html(infoDetailms == ''||infoDetailms==null ? '< 预览消息描述处 >': infoDetailms);
	...
	<form id="form30201" class="dlg-form" method="post" action="success.html" novalidate="novalidate">
		<table class="container">
			...
			<tr>
				...
				<td><textarea id="tsmsg" name="tsmsg" style="width:800px;height:350px;"></textarea></td>
			</tr>			
		</table>		
