...
$(function() {
	...
	$('#dg').datagrid({   
		...
		fitColumns:true,		
		method:'post',		
		url: "<%=request.getContextPath()%>/apiCtrl00101.json",
		columns:[[
			...
			//}},			
			{title:'组名',field:'name',align:'center',width:80,editor:'text'},			
			{title:'所属组',field:'parent',align:'center',width:100,editor:'text'}									
		]],
		toolbar: '#toolbar',
	...
	$('#b_add').click(function (){
		$('#dlg').dialog('open').dialog('setTitle','组基础信息配置');		
		$('#formtitle').text('组基础信息配置');
		$('#fm').form('clear');		
		url="<%=request.getContextPath()%>/apiCtrl00102.json"		
	});
	...
	$('#b_edit').click(function (){
		...
		if (row.length == 1){
			if (row){							
				$('#dlg').dialog('open').dialog('setTitle','组基础信息配置修改');
				$('#formtitle').text('组基础信息配置修改');
				$('#fm').form('load',row[0]);
				url = "<%=request.getContextPath()%>/apiCtrl00103.json";
			}
		}
		else $.messager.alert('提示', '请选中一条待修改组基础信息！', 'info' );
	});
	...
	$('#b_get_addon').click(function (){		
		...
		if (row.length == 1){
			if (row){							
				$('#dlg_addon').dialog('open').dialog('setTitle','组参数信息配置修改');
				$('#formtitle_addon').text('组参数信息配置修改');
				$('#fm').form('load',row[0]);
				if (!$("#fm").form("validate")) return;
				var arr = $('#fm').serializeArray();
				url = "<%=request.getContextPath()%>/apiCtrl00104.json?name="+row[0].name+"&parent="+row[0].parent;
				centerid = row[0].name;
				$('#dg_addon').datagrid({   
					iconCls: 'icon-edit',
					title:'参数信息',
		...
		else $.messager.alert('提示', '请选中一条待修改组参数信息！', 'info' );
	});
	
	$('#b_add_addon').click(function (){
		$('#dlg_addon_pare').dialog('open').dialog('setTitle','组参数信息配置添加');		
		$('#formtitle_addon_pare').text('组参数信息配置添加');
		$('#fm_addon_pare').form('clear');		
		url="<%=request.getContextPath()%>/apiCtrl00105.json?centerid="+centerid;		
	});
	...
	$('#b_edit_addon').click(function (){
		...
		if (row.length == 1){		
			if (row){							
				$('#dlg_addon_pare').dialog('open').dialog('setTitle','组参数信息配置修改');
				$('#formtitle_addon_pare').text('组参数信息配置修改');
		...
		if ($checkboxs.length > 0) {
			...
			$.messager.confirm('提示','确认是否删除?',function(r){
				if(r){
					$.ajax({
						type : "POST",
						url : "<%=request.getContextPath()%>/apiCtrl00106.json?centerid="+centerid+"&name="+code,
						dataType: "json",
						//data:arr,
						success : function(data) {
			...
			<form id="fm" class="dlg-form" method="post" novalidate="novalidate">
				<table class="dlgcontainer">
					...
					<tr>
						...
						<td>
							<input type="text" id="parent" name="parent" validType="length[0,200]"
								class="easyui-validatebox" style="width: 248px;"/>
						</td>
					</tr>					
				</table>
