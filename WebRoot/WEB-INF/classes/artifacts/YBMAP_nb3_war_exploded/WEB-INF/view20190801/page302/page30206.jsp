...
$(function() {
	...
	$('#dg').datagrid({   
		...
		height:369,
		singleSelect: true,
		url: "<%=request.getContextPath()%>/webapi12304.json",
		method:'post',
		...
		columns:[[
			{field:'ck',field:'checkbox',title:'删除',align:'center',formatter:function (value,row,index){
				return '<input type="checkbox" value="' + row.timeseqid + '"/>';
			}},
			...
			{title:'开始时间',field:'starttime',align:'center',width:130},
			//{title:'消息主题描述',field:'message_topic_type_name',align:'center',width:180,editor:'text'},					
			{title:'结束时间',field:'endtime',align:'center',width:120},
			{title:'序号',field:'num',hidden:'true',align:'center',width:10}
		]],
		toolbar: '#toolbar',
	...
	$('#b_add').click(function (){		
		$('#dlg').dialog('open').dialog('setTitle','免打扰时间增加');		
		$('#formtitle').text('免打扰时间增加');		
		$('#fm').form('clear');		
		url = "<%=request.getContextPath()%>/webapi12301.json";		
	});
	...
	$('#b_edit').click(function (){
		...
		if (row.length == 1){
			$('#dlg').dialog('open').dialog('setTitle','免打扰时间修改');
			$('#formtitle').text('免打扰时间修改');
			$('#fm').form('load',row[0]);
			url = "<%=request.getContextPath()%>/webapi12303.json";
		}
		else $.messager.alert('提示', '请选中一条待修改免打扰时间信息！', 'info' );
	});
		...
		if ($checkboxs.length > 0) {
			...
			$.messager.confirm('提示','确认是否删除?',function(r){
				if(r){
					$.ajax({
						type : "POST",
						url : "<%=request.getContextPath()%>/webapi12302.json",
						dataType: "json",
						data:arr,
						success : function(data) {
							if (data.recode == SUCCESS_CODE) {
	...
	$("#b_savesort").click(function(){
		...
		var total = options.total;
		 ...
		 for(var i=0;i<data.length;i++){
			...
			if(i!=0)param+=",";			
			param+="{'timeseqid':"+row['timeseqid']+",'num':"+(((curr-1)*options.pageSize)+i+1)+"}";
		 ...
		 param+="]";
		  ...
		  $.ajax( {
				type : 'POST',
				url : '<%=request.getContextPath()%>/webapi12305.json?datalist='+param,
				dataType: 'json',
				success : function(data) {
					$.messager.alert('提示', data.msg, 'info');
				},
...
<body>
<div class="easyui-panel" title="免打扰时间信息查询">
	<form id="form30205" class="dlg-form" novalidate="novalidate">
		<table class="container">	
			...
			<tr>
				...
				<td>
				<select id="centerid" name="centerid" class="easyui-combobox" editable="false" style="width:300px;" panelHeight="auto">
	...
	<form id="fm" class="dlg-form" method="post" novalidate="novalidate">
		<table class="dlgcontainer">
			...
			<tr>
				...
				<td>					
					<input id="starttime" name="starttime" required="true" class="easyui-timespinner" missingMessage="该输入项为必输项" style="width:230px;"/>
				</td>
			</tr>			
			<tr>		           
				<td><label for="endtime">结束时间:</label></td>
				<td>
					<input id="endtime" name="endtime" required="true" class="easyui-timespinner" missingMessage="该输入项为必输项" style="width:230px;"/>
					<input id="timeseqid" name="timeseqid" hidden="true" class="easyui-validatebox""/>					
				</td>
			</tr>	        
		  </table>
	</form>
