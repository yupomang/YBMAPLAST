...
<div class="easyui-panel" title="用户信息查询">
	<form id="form10305" class="dlg-form" novalidate="novalidate">
		...
		<div class="buttons">
			<a class="easyui-linkbutton" iconCls="icon-search" href="javascript:void(0)" onclick="select10305()">查询</a>
		</div>
	</form>
</div>
...
require(
    ...
    function (ec) {
		...
		var endvalue= myformatter(curr_time);
   	 	$("#enddate").datebox();
   	 	$("#enddate").datebox("setValue",endvalue);
   	 	$("#startdate").datebox();
   		$("#startdate").datebox("setValue", myformatter(new Date(new Date()-6*24*60*60*1000)));
   		var arr = $('#form10305').serializeArray();
        ...
        $.ajax({
	       		type : 'POST',
	       		url : '<%= _contexPath %>/webapi10305.json',
	       		dataType: 'json',
	       		data:map,
	       		success : function(data){
	       			...
	       			$('#dg').datagrid({   
	       				...
	       				//fitColumns:true,
	       				method:'post',
	       				url: "<%=request.getContextPath()%>/webapi1030501.json",
	       				queryParams:map,
	       				columns:[[
	       					{title:'时间',field:'appcreatedate',align:'center',width:110,editor:'text'},
...
function select10305() {
	if (!$('#form10305').form('validate')) return;
