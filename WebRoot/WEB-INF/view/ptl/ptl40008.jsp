...
$(function () {
	...
	$('#dg').datagrid({
		iconCls: 'icon-edit',
    	height:369,
		url: '<%= _contexPath %>/ptl40008Qry.json',
		method:'post',		
		title:'日志代码信息查询结果',
		toolbar:'#toolbar',
		pagination:true,
		...
		columns:[[
			{field:'id',field:'checkbox',title:'删除',align:'center',formatter:function (value,row,index){
				return '<input type="checkbox" value="' + row.logcode + '"/>';
			}},
			{title:'日志代码',field:'logcode',align:'center',width:30},
			{title:'日志内容',field:'logtext',align:'center',width:100}
		]],
...
var url;
function addInfo(){
    $('#dlg').dialog('open').dialog('setTitle','日志代码-增加');
    $("[name='logcode']").attr("readonly",false);
    $('#fm').form('clear');
    
    url = '<%= _contexPath %>/ptl40008Add.json';
}
function editInfo(){
    var row = $('#dg').datagrid('getSelected');
    if (row){
        $('#dlg').dialog('open').dialog('setTitle','日志代码-修改');
        $('#fm').form('load',row);
        $('input[name=logcode]').eq(1).attr("readonly",true);
        url = '<%= _contexPath %>/ptl40008Mod.json?id='+row.id;
    }else{
...
function delInfo(){
	...
	if ($checkboxs.length > 0) {
         $.messager.confirm('提示','确认是否删除?',function(r){
             if(r){
                //var para = rows[0].logcode;
             	//for(var i=1;i<rows.length;i++){
             	//	para = para + "," + rows[i].logcode;
             	//}
				var para = $checkboxs.map(function(index){
					return this.value;
				}).get().join(',');
				
            	var map={logcode:para}
            	
                $.ajax({
					type : 'POST',
					url : '<%= _contexPath %>/ptl40008Del.json',
					dataType: 'json',
					data:map,
					success : function(data) {
						if(SUCCESS_CODE == data.recode) {
...
function queryInfo() {
	//校验
	//if (!ydl.formValidate('form40008')) return;
	if (!$("#form40008").form("validate")) return;
	
	var $grid=$('#dg');
    $grid.datagrid('options').queryParams={}; 
    var queryParams = $grid.datagrid('options').queryParams; 
	$("#form40008 input").each(function(index){
		 if($(this).val()!="")
...
function batchUpload(){
    $('#dlgBatchUpload').dialog('open').dialog('setTitle','日志代码-批量导入');
}
function batchDownload(){
    $('#dlgBatchDownload').dialog('open').dialog('setTitle','日志代码-批量导出');
}
//]]></script>
</head>
<body>
	<div class="easyui-panel easyui-layout" title="日志代码信息">
		<form id="form40008" class="dlg-form" method="post" action="success.html" novalidate="novalidate">
			<table class="container">
				<col width="30%" /><col width="70%" />
				<tr>
					<th><label for="logcode">日志代码：</label></th>
					<td colspan="3">
   ...
   <div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        <div class="formtitle">日志代码信息</div>
        <form id="fm" method="post" class="dlg-form" novalidate="novalidate">
			<table class="dlgcontainer">
				<col width="30%" /><col width="70%" />
				<tr>
					<td><label for="logcode">日志代码：</label></td>
					<td><input type="text" id="logcode" name="logcode" class="easyui-validatebox" required="true" length="6"/></td>	        	
				</tr>
				<tr>
					<td><label for="logtext">日志内容：</label></td>
					<td><input type="text" id="logtext" name="logtext" class="easyui-validatebox" required="true"/></td>	        	
				</tr>
			</table>
        </form>
    	<div id="dlg-buttons">
    ...
    <div id="dlgBatchUpload" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        <div class="formtitle">日志代码信息批量导入</div>
        <form id = "uploadform" class="dlg-form" method="post"  ENCTYPE="multipart/form-data" novalidate="novalidate">
			...
			<input type="hidden" name="path" value="ptl/ptl40008"/>
    ...
    <div id="dlgBatchDownload" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
        <div class="formtitle">日志代码信息批量导出</div>
