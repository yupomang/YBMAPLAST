    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			//选择文件错误触发事件;
			webUploader.on('error', function( code ,file) {
				var text = '';
				switch( code ) {
					case  'F_DUPLICATE' : text = file.name + '该文件已经被选择了!' ;
					break;
					case  'Q_EXCEED_NUM_LIMIT' : text = '上传文件数量超过限制!' ;
					break;
					case  'F_EXCEED_SIZE' : text = file.name + '文件大小超过限制!';
					break;
					case  'Q_EXCEED_SIZE_LIMIT' : text = '所有文件总大小超过限制!';
					break;
					case 'Q_TYPE_DENIED' : text = file.name + '文件类型不正确或者是空文件!';
					break;
					default : text = '未知错误!';
 					break;	
				}
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			//文件上传方式
			swf:"/YBMAPZH/scripts/foddermanager/js/Uploader.swf",
			method:"POST",
			//服务器地址;
			server:"",
